...
import com.Res.Restaurant.utils.RestaurantUtils;
...
public class BillServiceImpl implements BillService {
        ...
        try {
            ...
            if(validateRequestMap(request)){
                if(request.containsKey("isGenerate") && !(Boolean)request.get("isGenerate")){
                    ...
                    fileName = RestaurantUtils.generateUUID() + jwtFilter.getCurrentUser();
                ...
                PdfWriter.getInstance(document, new FileOutputStream(MessageConstants.LOCATION + "\\" + fileName + ".pdf"));
                document.open();
                ...
                for(int i = 0; i < jsonArray.length(); i++){
                    addRows(pdfPTable, RestaurantUtils.getMapFromJson(jsonArray.getString(i)));
                }
            ...
            return ApiResponse.getResponseEntity("Some field is required!", HttpStatus.BAD_REQUEST);
        }
        catch (Exception e){
            e.printStackTrace();
        }
    ...
    public ResponseEntity<byte[]> getPdf(Map<String, Object> request) {
        try {
            ...
            String filePath = MessageConstants.LOCATION + "\\" + request.get("uuid") + ".pdf";
            if (RestaurantUtils.isExistFile(filePath)){
    ...
    public ResponseEntity<String> deleteBill(Integer id) {
        try {
            ...
            if(!bill.isEmpty()){
                ...
                return ApiResponse.getResponseEntity("Delete bill successfully", HttpStatus.OK);
            }
            return ApiResponse.getResponseEntity("This bill does not exist!", HttpStatus.NOT_FOUND);
        }
        catch (Exception e){
            e.printStackTrace();
        }
